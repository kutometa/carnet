...
testing_logic() {
    ...
    printf "%s\n" "extra" > "src/extra"
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test
    printf "%s\n" "^\./src/extra" >> ".carnet/seal-ignore.list"
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test
    printf "%s\n" "^\./\.carnet/seal-ignore\.list" >> ".carnet/seal-ignore.list"
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test
    printf "%s\n" ".*" >> ".carnet/seal-ignore.list"
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test
    
    
}

test_environment testing_logic   "Messing with validation" "-"
