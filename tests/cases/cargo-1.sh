...
testing_logic() {
    ...
    -- "[LINENO:$LINENO]" "testing cargo without a dev session"
    ACTIVE_USER="ORIGINAL_OWNER" 
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" build
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" check
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" clean
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" doc
    # TODO run "207" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" init
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" run
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" check
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" version
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" help
    
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" edit
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" build
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" check
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" clean
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" doc
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" run
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" check
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" version
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" help
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" done
...
test_environment testing_logic   "Testing cargo commands" "Testing cargo commands through carnet"
