...
testing_logic() {
    ...
    ACTIVE_USER="ORIGINAL_OWNER" 
    run "0" "$CARNET_PATH" --carnet:config-dir="../../configs/$ACTIVE_USER" init
    
    
    -- "ORIGINAL_OWNER edit crate"
    ACTIVE_USER="ORIGINAL_OWNER" 
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" edit
    
    -- "ORIGINAL_OWNER test crate"
    ACTIVE_USER="ORIGINAL_OWNER" 
    ...
    -- "Verify crate by the three users"
    ACTIVE_USER="CO-OWNER" 
    ...
    -- "ORIGINAL_OWNER untrust crate"
    ACTIVE_USER="ORIGINAL_OWNER" 
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" distrust < <(echo "")
    run "0" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" distrust < <(echo "yes")
    
    -- "ORIGINAL_OWNER test crate"
    ACTIVE_USER="ORIGINAL_OWNER" 
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test < <(echo "")
    run "0"   "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" test < <(echo "yes")
    
    
    
    -- "ORIGINAL_OWNER uninit crate"
    ACTIVE_USER="ORIGINAL_OWNER" 
    run "107" "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" uninit < <(echo "")
    run "0"   "$CARNET_PATH" --config-dir="../../configs/$ACTIVE_USER" uninit < <(echo "yes")
...
test_environment testing_logic   "Ownership 2" "Initializing a preexisting crate"
